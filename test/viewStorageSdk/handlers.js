...
import { mockServicesList, mockUploadKey, mockUploadList, updateBucketMetadata, mockEmbeddingResult } from '../viewStorageSdk/mockData';
import { mockTenantId } from '../setupTest';
import { mockBucketGUID, mockBucketData, mockTags, mockAcl, mockObjectKey, mockObjectData, mockUploadData, mockUploadPart } from './mockData';
...
export const handlers = [
    ...
    http.put(`${mockEndpoint}v1.0/tenants/${mockTenantId}/buckets/${mockBucketGUID}/uploads/${mockUploadKey}`, ({ request }) => {
        const searchParams = new URL(request.url).searchParams;
    ...
    http.delete(`${mockEndpoint}v1.0/tenants/${mockTenantId}/buckets/${mockBucketGUID}/uploads/${mockUploadKey}`, ({ request }) => {
        const searchParams = new URL(request.url).searchParams;
        if (searchParams.get('partNumber') === mockUploadPart.toString()) {
            return HttpResponse.text('true');
        }
    ...
    http.put(`${mockEndpoint}v1.0/tenants/${mockTenantId}/buckets/${mockBucketGUID}/uploads/${mockUploadKey}/parts`, ({ request }) => {
        const searchParams = new URL(request.url).searchParams;
        if (searchParams.get('partNumber') === mockUploadPart.toString()) {
            return HttpResponse.json(mockUploadData);
        }
    ...
    http.put(`${mockEndpoint}v1.0/tenants/${mockTenantId}/buckets/${mockBucketGUID}/objects/${mockObjectKey}`, ({ request }) => {
        const searchParams = new URL(request.url).searchParams;
    ...
    http.delete(`${mockEndpoint}v1.0/tenants/${mockTenantId}/buckets/${mockBucketGUID}/objects/${mockObjectKey}`, ({ request }) => {
        const searchParams = new URL(request.url).searchParams;
        if (searchParams.has('tags') || searchParams.has('acl')) {
            return HttpResponse.text('true');
        }
    ...
    http.get(`${mockEndpoint}v1.0/tenants/${mockTenantId}/buckets/${mockBucketGUID}/objects/${mockObjectKey}`, ({ request }) => {
        const searchParams = new URL(request.url).searchParams;
        if (searchParams.has('acl')) {
            return HttpResponse.json(mockAcl);
        }
