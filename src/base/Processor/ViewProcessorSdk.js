...
export default class ViewProcessorSdk extends ViewSdkBase {
  ...
  async processStorageServer(
    ...
    try {
      ...
      // Send the request using the post method, passing the serialized JSON
      const response = await this.post(url, jsonPayload, ProcessorResponse, token);
      ...
      if (response) {
        LoggerInstance.log(SeverityEnum.Debug, `Success from ${url}`);
        return response;
      } else {
        LoggerInstance.log(SeverityEnum.Warn, `Non-success from ${url}`);
   ...
   * @param {AbortSignal} token - Cancellation token.
   */
  async processCrawler(tenant, collection, repo, obj, mdRule, embedRule, vectorRepo, graphRepo, token = null) {
    if (!obj) throw new Error('Object metadata is required.');
    if (!mdRule) throw new Error('Metadata rule is required.');
   ...
   *
   */
  semanticCell = async (document, cancelToken) => {
    if (!document) {
    ...
    return await this.post(url, document, SemanticCellResponse, cancelToken);
  };
}
