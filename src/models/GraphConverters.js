export default class GraphConverters {
   ...
   */
  static lgGraphListToGraphList(graphs) {
    if (!graphs) return null;

    return graphs.map((graph) => GraphConverters.lgGraphToGraph(graph));
  }
   ...
   */
  static lgNodeListToGraphNodeList(nodes) {
   ...
   */
  static lgEdgeListToGraphEdgeList(edges) {
    if (!edges) return null;

    return edges.map((edge) => GraphConverters.lgEdgeToGraphEdge(edge));
  }
}
